import { Controller, Get, Post, Body, Patch, Param, Delete } from '@nestjs/common';
import { NagadService } from './nagad.service';
import { CreateNagadDto } from './dto/create-nagad.dto';
import { UpdateNagadDto } from './dto/update-nagad.dto';

@Controller('nagad')
export class NagadController {
  private readonly baseURL: 'http://sandbox.mynagad.com:10080';
	private readonly merchantID: 'MC084704';
	private readonly merchantNumber: ' 01400001104';
	private readonly pubKey: '';
	private readonly privKey: '';

	private readonly callbackURL: string;
  constructor(private readonly nagadService: NagadService) {}

  @Post()
  create(@Body() createNagadDto: CreateNagadDto) {
    return this.nagadService.create(createNagadDto);
  }

  @Get()
  findAll() {
    return this.nagadService.findAll();
  }

  @Get(':id')
  findOne(@Param('id') id: string) {
    return this.nagadService.findOne(+id);
  }

  @Patch(':id')
  update(@Param('id') id: string, @Body() updateNagadDto: UpdateNagadDto) {
    return this.nagadService.update(+id, updateNagadDto);
  }

  @Delete(':id')
  remove(@Param('id') id: string) {
    return this.nagadService.remove(+id);
  }
}
